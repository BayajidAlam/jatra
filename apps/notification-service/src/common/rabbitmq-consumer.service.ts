...
export class RabbitMQConsumerService implements OnModuleInit {
  ...
  private async connect() {
    try {
      ...
      this.logger.log(`✅ Connected to RabbitMQ and listening on queue: ${queue}`);
  ...
  private async handleMessage(msg: amqp.Message) {
    try {
      ...
      this.logger.log(`📩 Received event: ${event.type} for booking ${event.bookingId}`);
  ...
  private async processEvent(event: BookingEvent) {
    ...
    try {
      ...
      this.logger.log(`✅ Notification sent successfully for booking ${event.bookingId}`);
    } catch (error) {
      this.logger.error(`❌ Failed to process notification for booking ${event.bookingId}:`, error);
      throw error;
    }
  }
