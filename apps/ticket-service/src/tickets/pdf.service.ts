...
export class PDFService {
  ...
  private readonly storagePath = process.env.TICKET_PDF_STORAGE_PATH || './storage/tickets';
  ...
  async generateTicketPDF(data: TicketPDFData): Promise<string> {
    try {
      ...
      doc
        .text(`Date: ${data.journeyDate}`, 60, doc.y)
        .moveDown(0.5);
      ...
      doc
        .text(`Email: ${data.passengerEmail}`, 50, doc.y)
        .moveDown(0.5);

      doc
        .text(`Phone: ${data.passengerPhone}`, 50, doc.y)
        .moveDown(2);
      ...
      data.seats.forEach((seat) => {
        doc
          .fontSize(12)
          .fillColor('#000000')
          .text(`Coach: ${seat.coach} | Seat: ${seat.seatNumber} | Class: ${seat.class}`, 50, doc.y)
          .moveDown(0.5);
      });
      ...
      doc
        .text(`Payment Method: ${data.paymentMethod}`, 50, doc.y)
        .moveDown(2);
      ...
      doc
        .fontSize(10)
        .fillColor('#666666')
        .text('Please arrive at the station 30 minutes before departure.', { align: 'center' })
        .moveDown(0.3);

      doc
        .text('This is a computer-generated ticket and does not require a signature.', { align: 'center' })
        .moveDown(0.3);

      doc
        .text(`Booking ID: ${data.bookingId}`, { align: 'center' });
      ...
      const urlBase = process.env.TICKET_PDF_URL_BASE || 'http://localhost:3006/tickets';
